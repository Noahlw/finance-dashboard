...
4. [#71 — Wire `_requireOperator`/active-status authorization into 9 unguarded mutating endpoints](https://github.com/Noahlw/finance-dashboard/issues/71)
...
- [ ] **Wave B, claim and Event foundations:** #73, then #76. Event CRUD/list/picker work establishes the real Event selection surface; draft resume then restores saved Claim state into the existing form. #67 (legacy submitted-Claim Event re-link) is an adjacent follow-on and should land after #73 plus the Claim edit support it requires.
...
The order is recommended, not a license to change child-ticket scope. Agents may parallelize independent tickets within a wave, but must respect the #73 prerequisite for #76/#67's Event picker integration and must not declare a wave complete from compilation alone.
