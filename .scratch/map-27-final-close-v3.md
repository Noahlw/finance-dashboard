...
- Pre-launch blocker bar from #55/#59 (P0 status fix, 8 confirmed `_requireOperator`/active-status auth gaps from the original 9-entry inventory, with `api_resolveSession` verified as already guarded, Budget Request Reduce/Close UI, Discord retry UI, reconciliation drill-down UI, Reports filters, Payout account selector, P0-A upload race, P0-B draft-resume gap, Event CRUD).
