...
var Engine = {
  ...
  /** @private */
  _appendNote(sheet, rowIndex, colIndex, extra) {
  ...
  _applyBudgetRequestEffect(
    ...
    if (action === "APPROVE" || action === "REDUCE" || action === "REJECT") {
      ...
      sheet
        .getRange(row.rowIndex, c.decision_note)
        .setValue(payload.decision_note || "");
      ...
      try {
        FormSetup.refreshClaimFormChoices();
      } catch (e) {
        /* forms not set up yet */
      }
      ...
      sheet
        .getRange(row.rowIndex, c.decision_note)
        .setValue("Withdrawn by requester");
  ...
  _applyExpenseClaimEffect(
    ...
    if (action === "VERIFY") {
      ...
      sheet
        .getRange(row.rowIndex, c.total_amount)
        .setValue(Engine._sumClaimLineItems(claimId));
    } else if (action === "SUBMIT") {
      sheet.getRange(row.rowIndex, c.submitted_at).setValue(now);
      ...
      Engine._appendNote(
        ...
        row.rowIndex,
        c.notes,
        action + " by " + actorUserId + ": " + (payload.decision_note || "")
      );
    } else if (action === "RESUBMIT") {
      sheet.getRange(row.rowIndex, c.submitted_at).setValue(now);
    } else if (action === "LOCK") {
      ...
      var protection = sheet
        .getRange(row.rowIndex, 1, 1, sheet.getMaxColumns())
        .protect()
        .setDescription("LOCKED: " + claimId);
   ...
   * @private
   */
  _applyLineDecision(requestId, action, payload) {
    var sheet = getSheet_(TABS.BUDGET_REQUEST_LINES);
    var lines = Engine._findRowsByColumn(
      sheet,
      COLS.BudgetRequestLines.request_id,
      requestId
    );
    ...
    if (action === "APPROVE") {
      lines.forEach((line) => {
        ...
        sheet
          .getRange(line.rowIndex, c.line_status)
          .setValue(STATUS.BudgetRequestLine.APPROVED);
      });
      return;
    }
    if (action === "REJECT") {
      lines.forEach((line) => {
        ...
        sheet
          .getRange(line.rowIndex, c.line_status)
          .setValue(STATUS.BudgetRequestLine.REJECTED);
      });
      return;
    }
    // REDUCE: proportional split
    var requestedAmounts = lines.map(
      (l) => Number(l.values[c.requested_amount - 1]) || 0
    );
    var outcomes = CoreDecisions.computeReduceSplit(
      requestedAmounts,
      payload.amount_override
    );
    lines.forEach((line, i) => {
      sheet
        .getRange(line.rowIndex, c.approved_amount)
        .setValue(outcomes[i].approved_amount);
      sheet
        .getRange(line.rowIndex, c.line_status)
        .setValue(outcomes[i].line_status);
    });
  },
   ...
   */
  _computeEffectiveBalance(accountId) {
   ...
   * @private
   */
  _computeLockedRowHash(sheet, rowIndex) {
   ...
   * @private
   */
  _deleteDrafts(draftStatus, tabName, cols) {
  ...
  /** @private */
  _deny(entityType, entityId, action, actorUserId, reason, currentStatus) {
    Audit.append(
      actorUserId || "SYSTEM",
      entityType,
      entityId,
  ...
  _findCloseBlockers() {
    ...
    for (var i = 1; i < claimsValues.length; i++) {
      ...
      if (closedClaimStatuses.indexOf(cStatus) < 0) {
        blockers.push({
          id: cId,
          reason: "Unresolved claim: " + cStatus,
          type: "claim",
        });
      }
    }
    ...
    var closedReqStatuses = [
      STATUS.BudgetRequest.DRAFT,
      STATUS.BudgetRequest.APPROVED,
      STATUS.BudgetRequest.PARTIALLY_APPROVED,
      STATUS.BudgetRequest.REJECTED,
      STATUS.BudgetRequest.WITHDRAWN,
      STATUS.BudgetRequest.CLOSED,
    ];
    for (var j = 1; j < reqValues.length; j++) {
      ...
      if (closedReqStatuses.indexOf(rStatus) < 0) {
        blockers.push({
          id: rId,
          reason: "Unresolved budget request: " + rStatus,
          type: "budget_request",
        });
      }
    }
    ...
    for (var k = 1; k < payoutValues.length; k++) {
      ...
      if (payoutValues[k][pc.status - 1] === STATUS.Payout.QUEUED) {
        blockers.push({ id: pId, reason: "Queued payout", type: "payout" });
      }
    }
   ...
   * @private
   */
  _findIncomeByUuid(uuid) {
    var sheet = getSheet_(TABS.INCOME);
    var values = sheet.getDataRange().getValues();
    var c = COLS.Income;
    for (var i = 1; i < values.length; i++) {
      if (values[i][c.processed_response_id - 1] === uuid) {
        return values[i][c.income_id - 1];
      }
    }
    return null;
  },
   ...
   * @private
   */
  _findRowsByColumn(sheet, pkColIndex, matchValue) {
    var values = sheet.getDataRange().getValues();
    var out = [];
    for (var i = 1; i < values.length; i++) {
      if (values[i][pkColIndex - 1] === matchValue) {
        out.push({ rowIndex: i + 1, values: values[i] });
      }
    }
    return out;
  },
  ...
  /** @private */
  _findTransition(entityType, fromStatus, action) {
  ...
  _loadActor(userId) {
    ...
    return {
      displayName: row.values[COLS.Users.display_name - 1],
      role: row.values[COLS.Users.role - 1],
    };
  },
   ...
   * @private
   */
  _loadRow(entityType, id) {
    var tabMap = {
      BudgetRequest: TABS.BUDGET_REQUESTS,
      BudgetRequestLine: TABS.BUDGET_REQUEST_LINES,
      ClaimLineItem: TABS.CLAIM_LINE_ITEMS,
      ExpenseClaim: TABS.EXPENSE_CLAIMS,
  ...
  _notify(
    ...
    Discord.postStatus(entityId, title, toStatus, amount);
    if (selfApproved) {
      Discord.postSelfApproved(entityId, actorUserId, amount);
    }
  },

  /** @private */
  _ownerId(entityType, values) {
   ...
   * @private
   */
  _postToAccountBalance(accountId, amount, direction) {
    ...
    sheet
      .getRange(acctRow.rowIndex, ac.current_balance)
      .setValue(currentBalance);
    sheet
      .getRange(acctRow.rowIndex, ac.pending_income)
      .setValue(Math.max(pendingIncome, 0));
    sheet
      .getRange(acctRow.rowIndex, ac.reserved_payouts)
      .setValue(Math.max(reservedPayouts, 0));
  },
   ...
   * @private
   */
  _sumBudgetRequestLines(requestId, field) {
    var sheet = getSheet_(TABS.BUDGET_REQUEST_LINES);
    var rows = Engine._findRowsByColumn(
      sheet,
      COLS.BudgetRequestLines.request_id,
      requestId
    );
    ...
    return rows.reduce(
      (sum, r) => sum + (Number(r.values[colIndex - 1]) || 0),
      0
    );
  },
  ...
  _sumClaimedAgainstLine(budgetLineId) {
    ...
    cliRows.forEach((r) => {
      ...
      if (!(claimId in claimStatusCache)) {
        var claimRow = Engine._findRowsByColumn(
          claimSheet,
          COLS.ExpenseClaims.claim_id,
          claimId
        )[0];
        claimStatusCache[claimId] = claimRow
          ? claimRow.values[COLS.ExpenseClaims.status - 1]
          : null;
      }
  ...
  _sumClaimLineItems(claimId) {
    ...
    return rows.reduce(
      (sum, r) => sum + (Number(r.values[c.amount - 1]) || 0),
      0
    );
  },
   ...
   */
  _validateClaimVerification(claimId, actorUserId, payload) {
    var cliSheet = getSheet_(TABS.CLAIM_LINE_ITEMS);
    var cliRows = Engine._findRowsByColumn(
      cliSheet,
      COLS.ClaimLineItems.claim_id,
    ...
    for (var i = 0; i < cliRows.length; i++) {
      ...
      if (bLineRow) {
        var approved =
          Number(
            bLineRow.values[COLS.BudgetRequestLines.approved_amount - 1]
          ) || 0;
        ...
        if (remaining < 0) {
          return {
            ...
            reason:
              "Budget line " +
              bLineId +
              " over-claimed. Wait for top-up to be approved.",
          };
        }
      }
      ...
      if (!(rId || isMissing)) {
        return {
          ...
          reason:
            "Claim line " +
            cliRows[i].values[c.claim_line_id - 1] +
    ...
    if (receiptIdsToCheck.length > 0) {
      ...
      for (var j = 1; j < claimData.length; j++) {
        claimStatusMap[claimData[j][0]] =
          claimData[j][COLS.ExpenseClaims.status - 1];
      }
      ...
      for (var k = 1; k < allCliData.length; k++) {
        ...
        if (receiptIdsToCheck.indexOf(cliReceiptId) !== -1) {
          ...
          if (claimStatusMap[cliClaimId] !== STATUS.ExpenseClaim.REJECTED) {
            if (!allReceiptMap[cliReceiptId]) {
              allReceiptMap[cliReceiptId] = 0;
            }
            allReceiptMap[cliReceiptId] +=
              Number(allCliData[k][c.amount - 1]) || 0;
          }
        }
      }
      ...
      for (var r = 0; r < receiptIdsToCheck.length; r++) {
        ...
        if (receiptRow) {
          var printedTotal =
            Number(receiptRow.values[COLS.Receipts.receipt_total - 1]) || 0;
          ...
          if (totalClaimedAgainstIt > printedTotal) {
            return {
              ...
              reason:
                ...
                totalClaimedAgainstIt +
                ") across all claims for receipt " +
                checkReceiptId +
                " exceeds printed receipt total (" +
                printedTotal +
                ").",
            };
          }
          ...
          return {
            ok: false,
            reason: "Receipt " + checkReceiptId + " not found in Receipts tab.",
          };
        }
      }
    }
    ...
    if (hasMissingReceipt) {
      ...
      if (actor.role !== ROLES.TREASURER) {
        return {
          ok: false,
          reason: "Missing receipt claims must be verified by the TREASURER.",
        };
      }
      ...
      if (note.indexOf("EXCEPTION_GRANTED") === -1) {
        ...
        if (totalMissingAmount > cap) {
          return {
            ...
            reason:
              ...
              ") exceeds cap (" +
              cap +
              "). To bypass, type EXCEPTION_GRANTED in note.",
          };
        }
        ...
        for (var k = 1; k < allCliData.length; k++) {
          ...
          if (
            cInfo.claimant === claimantId &&
            cInfo.status !== STATUS.ExpenseClaim.REJECTED &&
            allCliData[k][c.missing_receipt_flag - 1] === true
          ) {
            count++;
          }
        }
        ...
        if (count >= maxPerSem) {
          return {
            ok: false,
            reason:
              "Claimant has exceeded missing receipt limit (" +
              maxPerSem +
              " per sem). To bypass, type EXCEPTION_GRANTED in note.",
          };
        }
      }
    }
   ...
   */
  adjustAccount(accountId, amount, direction, reason, actorUserId) {
    ...
    try {
      ...
      if (
        acctRow.values[COLS.FinanceAccounts.status - 1] !==
      ...
      return { adjustmentId: adjId, ok: true };
    } finally {
      lock.releaseLock();
    }
  },
  ...
  closeSemester(semester, actorUserId) {
    ...
    if (semester !== currentSemester) {
      return {
        ok: false,
        reason: "Can only close the current semester: " + currentSemester,
      };
    }
    ...
    if (blockers.length > 0) {
      return {
        blockers,
        ok: false,
        reason: blockers.length + " blocker(s) prevent close",
      };
    }
    ...
    try {
      ...
      for (var i = 1; i < accValues.length; i++) {
        if (!accValues[i][ac.account_id - 1]) {
          continue;
        }
      ...
      var nextSemester =
        currentIdx < semesters.length - 1 ? semesters[currentIdx + 1] : null;
      ...
      try {
        Discord.postTreasury(
          "📅 Final semester **" +
            currentSemester +
            "** closed. Annual Migration is now available."
        );
      } catch (e) {}
      return {
        closed: currentSemester,
   ...
   */
  computeCurrentLockedHash(claimId) {
   ...
   */
  confirmIncome(incomeId, accountId, actorUserId) {
    var lock = LockService.getScriptLock();
    try {
      lock.waitLock(30_000);
    } catch (e) {
    ...
    try {
      ...
      if (!actor || actor.role !== ROLES.TREASURER) {
        return {
          accountId: null,
          ok: false,
          reason: "Only a Treasurer can confirm income.",
        };
      }

      var row = Engine._loadRow("Income", incomeId);
      if (!row) {
        return { accountId: null, ok: false, reason: "Income not found." };
      }
      ...
      if (currentStatus === STATUS.Income.CONFIRMED) {
        return {
          accountId: row.values[c.account_id - 1] || accountId,
          ok: true,
          reason: null,
        };
      }
      if (
        currentStatus !== STATUS.Income.PENDING &&
        currentStatus !== STATUS.Income.NEEDS_INFO
      ) {
        return {
          accountId: null,
          ok: false,
          reason: "Income is " + currentStatus + ", not PENDING.",
        };
      }
      ...
      if (!accountId) {
        ...
        if (!accountId) {
          return {
            ...
            ok: false,
            reason:
              "No account_id specified and income has no proposed account.",
          };
        }
      }
      ...
      var acctRow = Engine._loadRow("FinanceAccount", accountId);
      if (!acctRow) {
        return { accountId: null, ok: false, reason: "Account not found." };
      }
      if (
        acctRow.values[COLS.FinanceAccounts.status - 1] !==
        STATUS.FinanceAccount.ACTIVE
      ) {
        return { accountId: null, ok: false, reason: "Account is not active." };
      }
      ...
      return { accountId, ok: true, reason: null };
    } finally {
      lock.releaseLock();
    }
  },
   ...
   */
  correctSemester(entityType, entityId, newSemester, actorUserId) {
    var row = Engine._loadRow(entityType, entityId);
    if (!row) {
    ...
    if (!cols.semester) {
      return { ok: false, reason: "Semester not applicable for " + entityType };
    }

    var currentSemester = row.values[cols.semester - 1];
    if (currentSemester === newSemester) {
      return { ok: true, reason: "Already assigned to " + newSemester };
    }
    ...
    return { from: currentSemester, ok: true, to: newSemester };
  },
  ...
  getSemesterStatus() {
    var currentSemester = Config.getOptional("CURRENT_SEMESTER") || "26A";
    var semStart =
      Config.getOptional("SEM_" + currentSemester + "_START") || "";
  ...
  recordIncome(
    ...
    try {
      ...
      Discord.postTreasury(
        "🚨 CRITICAL: Script lock timeout in Engine.recordIncome"
      );
      throw e;
    }
    try {
      if (uuid && Engine._alreadyProcessedIncome(uuid)) {
        return { incomeId: Engine._findIncomeByUuid(uuid), ok: true };
      }
      ...
      Audit.append(actorUserId, "Income", incomeId, "CREATE", {
        accountId: accountId || null,
        amount,
        sourceRef,
      });
      return { incomeId, ok: true };
    } finally {
      lock.releaseLock();
    }
  },
   ...
   */
  rejectIncome(incomeId, actorUserId, reason) {
    var row = Engine._loadRow("Income", incomeId);
    if (!row) {
      return { ok: false, reason: "Income not found." };
    }
    var c = COLS.Income;
    var currentStatus = row.values[c.status - 1];
    if (
      ...
      return {
        ok: false,
        reason: "Income is " + currentStatus + ", cannot reject.",
      };
    }
    var now = Audit._nowIso();
    ...
    Audit.append(actorUserId, "Income", incomeId, "REJECT", { reason });
    Discord.postTreasury(
      "**" +
        incomeId +
        "** — rejected by " +
   ...
   */
  requestIncomeInfo(incomeId, actorUserId, reason) {
    var row = Engine._loadRow("Income", incomeId);
    if (!row) {
      return { ok: false, reason: "Income not found." };
    }
    ...
    if (currentStatus !== STATUS.Income.PENDING) {
      return {
        ok: false,
        reason: "Income is " + currentStatus + ", cannot request info.",
      };
    }
    var now = Audit._nowIso();
    row.sheet
      .getRange(row.rowIndex, c.status)
      .setValue(STATUS.Income.NEEDS_INFO);
    row.sheet.getRange(row.rowIndex, c.decided_by).setValue(actorUserId);
    row.sheet.getRange(row.rowIndex, c.decided_at).setValue(now);
    row.sheet.getRange(row.rowIndex, c.decision_note).setValue(reason || "");
    Audit.append(actorUserId, "Income", incomeId, "REQUEST_INFO", { reason });
    return { ok: true };
  },
  ...
  transferBetweenAccounts(
    ...
    try {
      ...
      if (!actor || actor.role !== ROLES.TREASURER) {
        return {
          ok: false,
          reason: "Only a Treasurer can transfer between accounts.",
        };
      }
      ...
      if (
        fromRow.values[COLS.FinanceAccounts.status - 1] !==
        STATUS.FinanceAccount.ACTIVE
      ) {
        return { ok: false, reason: "Source account is not active." };
      }
      if (
        (Number(fromRow.values[COLS.FinanceAccounts.current_balance - 1]) ||
          0) < amount
      ) {
      ...
      if (
        toRow.values[COLS.FinanceAccounts.status - 1] !==
        STATUS.FinanceAccount.ACTIVE
      ) {
  ...
  transition(entityType, entityId, action, actorUserId, payload) {
    ...
    try {
      ...
      Discord.postTreasury(
        "🚨 CRITICAL: Script lock timeout in Engine.transition"
      );
      throw e;
    }
    try {
      var actor =
        actorUserId === "SYSTEM"
          ? { displayName: "System", role: ROLES.TREASURER }
          : Engine._loadActor(actorUserId);
      if (!actor) {
        return Engine._deny(
          entityType,
          entityId,
      ...
      if (entityType === "ExpenseClaim" && action === "VERIFY") {
        var verifyCheck = Engine._validateClaimVerification(
          entityId,
          actorUserId,
          payload
        );
      ...
      var nextStatus =
        entityType === "BudgetRequest"
          ? Engine._applyBudgetRequestEffect(
              entityId,
              row,
              action,
              def,
          ...
          : Engine._applyExpenseClaimEffect(
              entityId,
              row,
              action,
              def,
      ...
      if (effectDetail.lockedHash) {
        auditDetail.lockedHash = effectDetail.lockedHash;
      }
      Audit.append(
        actorUserId,
        entityType,
   ...
   */
  validateClaimLineAmount(budgetLineId, amount) {
...
function Engine_deriveRequestStatus(requestId) {
  ...
  var c = COLS.BudgetRequestLines;
  var statuses = rows.map((r) => r.values[c.line_status - 1]);
