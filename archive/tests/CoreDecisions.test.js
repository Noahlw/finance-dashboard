...
describe("findTransition", () => {
  test("BudgetRequest DRAFT --SUBMIT--> PENDING is legal and self-only", () => {
    var def = CoreDecisions.findTransition(
      "BudgetRequest",
...
describe("deriveRequestStatusFromLineStatuses", () => {
  test("empty line list stays PENDING (nothing to derive from yet)", () => {
    expect(CoreDecisions.deriveRequestStatusFromLineStatuses([])).toBe(
      STATUS.BudgetRequest.PENDING
    );
  });
  ...
  test("all lines APPROVED derives request APPROVED", () => {
    ...
    expect(CoreDecisions.deriveRequestStatusFromLineStatuses(statuses)).toBe(
      STATUS.BudgetRequest.APPROVED
    );
  });
  ...
  test("all lines REJECTED derives request REJECTED", () => {
    var statuses = [
      STATUS.BudgetRequestLine.REJECTED,
      STATUS.BudgetRequestLine.REJECTED,
    ];
    expect(CoreDecisions.deriveRequestStatusFromLineStatuses(statuses)).toBe(
  ...
  test("a REDUCED line among approved ones also derives PARTIALLY_APPROVED", () => {
    var statuses = [
      STATUS.BudgetRequestLine.APPROVED,
      STATUS.BudgetRequestLine.REDUCED,
    ];
    expect(CoreDecisions.deriveRequestStatusFromLineStatuses(statuses)).toBe(
      STATUS.BudgetRequest.PARTIALLY_APPROVED
    );
...
describe("SELF_APPROVAL_ACTIONS / isSelfApproval", () => {
  test("the self-approval action list matches D5", () => {
    expect(CoreDecisions.SELF_APPROVAL_ACTIONS).toEqual([
      "APPROVE",
      "REDUCE",
      "VERIFY",
      "APPROVE_PAYOUT",
...
describe("authorize", () => {
  ...
  test("self-only transition denies a non-owner regardless of role", () => {
    var result = CoreDecisions.authorize(
      selfOnlyDef,
      false,
      ROLES.TREASURER,
      {}
  ...
  test("self-only transition allows the owner", () => {
    expect(
      CoreDecisions.authorize(selfOnlyDef, true, ROLES.MEMBER, {}).ok
    ).toBe(true);
  });
  ...
  test("role-gated transition allows an allowed role", () => {
    expect(
      CoreDecisions.authorize(roleOnlyDef, false, ROLES.TREASURER, {}).ok
    ).toBe(true);
  });
  ...
  test("requiresNote allows a non-empty decision_note", () => {
    expect(
      CoreDecisions.authorize(noteDef, false, ROLES.TREASURER, {
        decision_note: "ok",
      }).ok
    ).toBe(true);
  });
...
describe("checkClaimLineAmount", () => {
  test("amount under remaining is ok", () => {
    expect(CoreDecisions.checkClaimLineAmount(50, 100, 40)).toEqual({
      ok: true,
      remaining: 60,
    });
  });
  ...
  test("amount exactly equal to remaining is ok (boundary, <=)", () => {
    expect(CoreDecisions.checkClaimLineAmount(60, 100, 40)).toEqual({
      ok: true,
      remaining: 60,
    });
  });
...
describe("computeReduceSplit", () => {
  ...
  test("a three-way tie never drifts a cent short of the override (largest-remainder allocation)", () => {
    ...
    var sum = result.reduce((s, r) => s + r.approved_amount, 0);
...
describe("resolveTreasurerIdDrift", () => {
  test("configured ID resolves to a real treasurer -> ok, no correction", () => {
    var result = CoreDecisions.resolveTreasurerIdDrift("USER-0001", [
      "USER-0001",
    ]);
  ...
  test("configured ID matches one of several real treasurers -> ok, no correction", () => {
    var result = CoreDecisions.resolveTreasurerIdDrift("USER-0002", [
      "USER-0001",
      "USER-0002",
    ]);
    expect(result).toEqual({ action: "ok", correctedId: null });
...
describe("parseClaimLines", () => {
  test("all three lines present and valid returns all three in order", () => {
    var answers = {
      "Line 1 — Amount (HKD)": "50",
      "Line 1 — Budget line":
        "BUDGETLINE-26A-001-01 — BBQ — remaining HK$300.00",
      "Line 2 — Amount (HKD)": "60.5",
      "Line 2 — Budget line":
        "BUDGETLINE-26A-001-02 — Drinks — remaining HK$150.00",
      "Line 3 — Amount (HKD)": "70.25",
      "Line 3 — Budget line":
        "BUDGETLINE-26A-001-03 — Rent — remaining HK$1000.00",
    };
    var result = CoreDecisions.parseClaimLines(answers);
    expect(result.length).toBe(3);
    expect(result[0]).toEqual({
      amount: 50,
      budgetLineChoice: "BUDGETLINE-26A-001-01 — BBQ — remaining HK$300.00",
      n: 1,
    });
    expect(result[1]).toEqual({
      amount: 60.5,
      budgetLineChoice: "BUDGETLINE-26A-001-02 — Drinks — remaining HK$150.00",
      n: 2,
    });
    expect(result[2]).toEqual({
      amount: 70.25,
      budgetLineChoice: "BUDGETLINE-26A-001-03 — Rent — remaining HK$1000.00",
      n: 3,
    });
  });
