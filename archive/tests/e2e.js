...
async function clickSubmit(page) {
  ...
  if (!clicked) {
    console.log('Could not find Submit button. Clicking the last button as fallback.');
...
async function checkSubmissionSuccess(page) {
  ...
  const errors = await page.$$eval('div[role="alert"]', els => els.map(e => e.innerText).filter(t => t.trim() !== ''));
  if (errors.length > 0) {
...
async function testBudgetRequest(baseUrl, formUrls) {
  ...
  try {
    ...
    await page.goto(formUrls.requestFormUrl, { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    if (inputs.length > 2) {
      ...
      await inputs[2].type('This is an automated E2E test using Puppeteer.', { delay: 30 });
    }
    // 3: Date
    if (inputs.length > 3) {
...
async function testExpenseClaim(baseUrl, formUrls) {
  ...
  try {
    const page = await browser.newPage();
    await page.goto(formUrls.claimFormUrl, { waitUntil: 'networkidle2', timeout: 30000 });
    await page.waitForSelector('input[type="email"]', { timeout: 10000 });
...
async function testOnboarding(baseUrl, formUrls) {
  ...
  try {
    const page = await browser.newPage();
    await page.goto(formUrls.onboardingFormUrl, { waitUntil: 'networkidle2', timeout: 30000 });
    await page.waitForSelector('input[type="email"]', { timeout: 10000 });
    ...
    if (allInputsAfter.length > 3) {
      ...
      await allInputsAfter[handleIdx].type('puppeteer-fps-handle', { delay: 30 });
    }
...
(async () => {
  ...
  if (scenario === 'all' || scenario === 'request') {
    scenarios.push({ name: 'Budget Request', fn: () => testBudgetRequest(baseUrl, formUrls) });
  }
  if (scenario === 'all' || scenario === 'claim') {
    scenarios.push({ name: 'Expense Claim', fn: () => testExpenseClaim(baseUrl, formUrls) });
  }
  if (scenario === 'all' || scenario === 'onboarding') {
    scenarios.push({ name: 'Committee Onboarding', fn: () => testOnboarding(baseUrl, formUrls) });
  }

  if (scenarios.length === 0) {
    console.error(`Unknown scenario: "${scenario}". Use "request", "claim", "onboarding", or omit for all.`);
    process.exit(1);
  }
