...
function onFormSubmitOnboarding(e) {
  ...
  try {
    ...
    Discord.postTreasury('🚨 CRITICAL: Script lock timeout in onFormSubmitOnboarding');
    throw err;
  }
  try {
    ...
    Audit.append(user.userId, 'User', user.userId, 'ONBOARDED', {
      userId: user.userId, isNew: user.isNew, role: user.role, source: 'ONBOARDING_FORM'
    });

    if (user.isNew) {
      Discord.postTreasury('⚠️ New COMMITTEE user created via Onboarding form: ' + user.userId +
        ' (' + email + ') — verify this was an expected exco addition.');
    }
  } finally {
    lock.releaseLock();
  }
...
function Onboarding_resolveOrCreateUser_(email, displayName) {
  ...
  for (var i = 1; i < values.length; i++) {
    if (String(values[i][c.email - 1]).toLowerCase() === email.toLowerCase()) {
      return { userId: values[i][c.user_id - 1], isNew: false, role: values[i][c.role - 1] };
    }
  }
  var userId = Ids.nextId('User');
  var now = Audit._nowIso();
  sheet.appendRow([userId, displayName, ROLES.COMMITTEE, email, true, now]);
  return { userId: userId, isNew: true, role: ROLES.COMMITTEE };
}
...
function Onboarding_writeVaultRow_(userId, fields) {
  ...
  var values = sheet.getDataRange().getValues();
  var c = COLS.Vault;
  var row = [userId, fields.full_name, fields.student_id, fields.payout_method, fields.payout_handle, fields.consent_ts];

  var targetRow;
  for (var i = 1; i < values.length; i++) {
    if (values[i][c.user_id - 1] === userId) { targetRow = i + 1; break; }
  }
  if (!targetRow) {
    sheet.appendRow(row);
  ...
  sheet.getRange(targetRow, c.student_id).setNumberFormat('@').setValue(fields.student_id);
  sheet.getRange(targetRow, c.payout_handle).setNumberFormat('@').setValue(fields.payout_handle);
}
