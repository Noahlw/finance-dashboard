...
var Ids = {
   ...
   * @private
   */
  _formatId: function (entityType, n) {
    var prefix = ENTITY_PREFIX[entityType];
    if (!prefix) throw new Error('Unknown entity type for ID generation: ' + entityType);
    var padded = Ids._padNumber(n, entityType);
    var semesterScoped = (entityType === 'Event' || entityType === 'BudgetRequest' ||
      entityType === 'ExpenseClaim' || entityType === 'Income' || entityType === 'Payout');
    if (semesterScoped) {
   ...
   * @private
   */
  _padNumber: function (n, entityType) {
