...
function getLedger_() {
  ...
  if (!id) throw new Error('LEDGER_ID not set in Script Properties. Run Setup.setupAll() first.');
...
function getVault_() {
  ...
  if (!id) throw new Error('VAULT_ID not set in Script Properties. Run Setup.setupAll() first.');
  return SpreadsheetApp.openById(id);
}
...
function getSheet_(tabName) {
  ...
  if (!sh) throw new Error('Tab not found in CF-Ledger: ' + tabName);
  return sh;
}
...
function getVaultSheet_() {
  ...
  if (!sh) throw new Error('Vault tab not found in CF-Vault.');
  return sh;
}
...
var Config = {
  ...
  get: function (key) {
    ...
    if (!Object.prototype.hasOwnProperty.call(map, key) || map[key] === '' || map[key] === 'PASTE_ME') {
      throw new Error('Config key missing/unset: ' + key + '. Set it in the Config tab.');
    }
  ...
  getNum: function (key) {
    ...
    var n = Number(raw);
    if (isNaN(n)) throw new Error('Config key is not a number: ' + key + ' = ' + raw);
    return n;
  },
  ...
  getOptional: function (key) {
    var map = Config._readAll();
    var val = map[key];
    if (!val || val === 'PASTE_ME') return null;
    return String(val);
  }
};
