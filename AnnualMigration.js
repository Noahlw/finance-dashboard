...
var Migration = {
  ...
  getState: function () {
    ...
    if (!stage || MIGRATION_STAGES.indexOf(stage) < 0) return null;

    var targetSpreadsheetId = Config.getOptional('MIGRATION_TARGET_SPREADSHEET_ID') || '';
  ...
  getPreview: function () {
    ...
    for (var i = 1; i < usersValues.length; i++) {
      ...
      var active = String(usersValues[i][uc.active - 1]).trim().toUpperCase() === 'TRUE';
      var entry = {
      ...
      if (role === ROLES.MEMBER) {
        if (active) activeMembers.push(entry);
        else inactiveMembers.push(entry);
    ...
    for (var j = 1; j < faValues.length; j++) {
      if (!faValues[j][fac.account_id - 1]) continue;
      accounts.push({
    ...
    for (var k = 1; k < catValues.length; k++) {
      if (!catValues[k][cc.category_id - 1]) continue;
      categories.push({
    ...
    for (var l = 1; l < evtValues.length; l++) {
      if (!evtValues[l][ec.event_id - 1]) continue;
      events.push({
    ...
    var yearNum = parseInt(currentYear.replace(/[^0-9]/g, ''), 10) || 26;
    var nextYearNum = yearNum + 1;

    return {
      current_year: currentYear,
      next_committee_year: nextYearNum,
      year_label: (nextYearNum - 1) + '-' + nextYearNum + ' (' + nextYearNum + ')',
      ...
      events: events,
      has_treasurer: operators.some(function(o) { return o.role === ROLES.TREASURER && o.active; })
    };
  },
   ...
   */
  startMigration: function (actorUserId) {
    ...
    if (!parentFolderId) {
      ...
      var currentFileId = PropertiesService.getScriptProperties().getProperty('LEDGER_ID') || '';
      if (currentFileId) {
        try {
    ...
    try {
      ...
      return { ok: false, reason: 'Failed to create year folder: ' + e.message };
    }
    ...
    var newSpreadsheet = SpreadsheetApp.create('CF-Budget ' + preview.year_label);
    ...
    Migration._setConfig('MIGRATION_COMMITTEE_YEAR', String(preview.next_committee_year));
   ...
   */
  setSelections: function (actorUserId, selections) {
    ...
    Audit.append(actorUserId, 'Migration', Config.getOptional('MIGRATION_YEAR_LABEL') || '', 'SELECTIONS_SET', {
   ...
   */
  executeMigration: function (actorUserId) {
    var targetSpreadsheetId = Config.getOptional('MIGRATION_TARGET_SPREADSHEET_ID') || '';
    if (!targetSpreadsheetId) return { ok: false, reason: 'No migration target spreadsheet' };
    ...
    try {
      ...
      for (var si = 0; si < sheets.length; si++) {
        if (sheets[si].getName() === 'Sheet1') {
          try { ss.deleteSheet(sheets[si]); } catch (e) {}
        }
      }

      // Create all tabs identical to Setup.js schema (simplified)
      Migration._createTab(ss, TABS.USERS, ['user_id', 'display_name', 'role', 'email', 'active', 'created_at']);
      Migration._createTab(ss, TABS.CATEGORIES, ['category_id', 'name', 'kind', 'semester_cap', 'active']);
      Migration._createTab(ss, TABS.EVENTS, ['event_id', 'name', 'semester', 'owner_user_id', 'created_at']);
      Migration._createTab(ss, TABS.BUDGET_REQUESTS, ['request_id', 'requester_id', 'event_id', 'title', 'justification', 'needed_by', 'status', 'submitted_at', 'decided_at', 'decided_by', 'decision_note', 'self_approved', 'processed_response_id']);
      Migration._createTab(ss, TABS.BUDGET_REQUEST_LINES, ['line_id', 'request_id', 'category_id', 'description', 'requested_amount', 'approved_amount', 'line_status', 'claimed_amount', 'remaining']);
      Migration._createTab(ss, TABS.EXPENSE_CLAIMS, ['claim_id', 'claimant_id', 'status', 'submitted_at', 'verified_at', 'approved_at', 'paid_at', 'locked_at', 'verified_by', 'approved_by', 'total_amount', 'late_flag', 'self_approved', 'notes', 'processed_response_id', 'created_by', 'expense_date', 'semester', 'event_id', 'payout_method', 'payout_handle']);
      Migration._createTab(ss, TABS.CLAIM_LINE_ITEMS, ['claim_line_id', 'claim_id', 'budget_line_id', 'receipt_id', 'amount', 'description', 'missing_receipt_flag']);
      Migration._createTab(ss, TABS.RECEIPTS, ['receipt_id', 'drive_file_id', 'sha256', 'uploaded_by', 'uploaded_at', 'vendor', 'receipt_date', 'receipt_total', 'file_link']);
      Migration._createTab(ss, TABS.INCOME, ['income_id', 'date', 'category_id', 'amount', 'received_by', 'source_ref', 'event_id', 'notes', 'account_id', 'status', 'decided_by', 'decided_at', 'decision_note', 'processed_response_id']);
      Migration._createTab(ss, TABS.PAYOUTS, ['payout_id', 'claim_id', 'payee_user_id', 'amount', 'method', 'txn_reference', 'paid_by', 'status', 'paid_at', 'confirmed_at', 'account_id', 'failure_reason', 'parent_payout_id']);
      Migration._createTab(ss, TABS.FINANCE_ACCOUNTS, ['account_id', 'name', 'opening_balance', 'current_balance', 'pending_income', 'reserved_payouts', 'status', 'created_at', 'deactivated_at']);
      Migration._createTab(ss, TABS.ACCOUNT_TRANSFERS, ['transfer_id', 'from_account_id', 'to_account_id', 'amount', 'reason', 'transferred_by', 'transferred_at']);
      Migration._createTab(ss, TABS.ACCOUNT_ADJUSTMENTS, ['adjustment_id', 'account_id', 'amount', 'direction', 'reason', 'adjusted_by', 'adjusted_at']);
      Migration._createTab(ss, TABS.AUDIT_LOG, ['seq', 'ts', 'actor_user_id', 'entity_type', 'entity_id', 'action', 'detail', 'prev_hash', 'row_hash']);
      Migration._createTab(ss, TABS.APPROVALS, ['entity_id', 'entity_type', 'title', 'requester_or_claimant', 'amount', 'status', 'action', 'amount_override', 'note', 'confirm', 'intent_actor_email', 'receipt_link']);
      ...
      preview.active_members.forEach(function(m) {
        if (memberIds.indexOf(m.user_id) >= 0) memberMap[m.user_id] = m;
      });
      preview.inactive_members.forEach(function(m) {
        if (memberIds.indexOf(m.user_id) >= 0) memberMap[m.user_id] = m;
      });
      preview.operators.forEach(function(op) {
        memberMap[op.user_id] = op;
      });

      Object.keys(memberMap).forEach(function(uid) {
        var m = memberMap[uid];
        usersSheet.appendRow([uid, m.display_name, m.role, m.email || '', m.active, now]);
      });
      ...
      preview.accounts.forEach(function(acct) {
        if (accountIds.indexOf(acct.account_id) >= 0) {
          var openingBalance = Number(balances[acct.account_id]) || acct.current_balance;
          accountsSheet.appendRow([
            acct.account_id, acct.name, openingBalance, openingBalance, 0, 0, ROLES.COMMITTEE ? STATUS.FinanceAccount.ACTIVE : 'ACTIVE', now, ''
          ]);
        } else {
          // Inactive reference for non-carried accounts
          accountsSheet.appendRow([
            acct.account_id, acct.name + ' (closed)', 0, 0, 0, 0, STATUS.FinanceAccount.INACTIVE, now, now
          ]);
        }
      });
      ...
      preview.categories.forEach(function(cat) {
        if (catIds.indexOf(cat.category_id) >= 0) {
          catSheet.appendRow([cat.category_id, cat.name, cat.kind, 0, cat.active]);
        }
      });
      ...
      var eventCount = 0;
      preview.events.forEach(function(evt) {
        if (eventIds.indexOf(evt.event_id) >= 0) {
          eventCount++;
          evtSheet.appendRow([evt.event_id, evt.name + ' (' + (Number(preview.next_committee_year) - 1) + ')', preview.current_year, preview.operators.length > 0 ? preview.operators[0].user_id : '', now]);
        }
      });
      ...
      configSheet.appendRow(['COMMITTEE_YEAR', String(preview.next_committee_year || 27)]);

      // Seed Counters
      var countersSheet = ss.getSheetByName(TABS.COUNTERS);
      ['User', 'Event', 'BudgetRequest', 'BudgetRequestLine', 'ExpenseClaim', 'ClaimLineItem', 'Receipt', 'Income', 'Payout', 'FinanceAccount', 'AccountTransfer', 'AccountAdjustment'].forEach(function(entity) {
      ...
      return { ok: true, stage: 'REVIEW', target_spreadsheet_id: targetSpreadsheetId };
    } catch (e) {
   ...
   */
  activateMigration: function (actorUserId) {
    var stage = Config.getOptional('MIGRATION_STAGE') || '';
    if (stage !== 'REVIEW') return { ok: false, reason: 'Migration must be in REVIEW stage. Current: ' + stage };

    var targetSpreadsheetId = Config.getOptional('MIGRATION_TARGET_SPREADSHEET_ID') || '';
    if (!targetSpreadsheetId) return { ok: false, reason: 'No target spreadsheet' };

    var oldSpreadsheetId = PropertiesService.getScriptProperties().getProperty('LEDGER_ID') || '';

    // Set the new spreadsheet as active
    PropertiesService.getScriptProperties().setProperty('LEDGER_ID', targetSpreadsheetId);
    ...
    try { Discord.postTreasury('🎉 Annual Migration activated! New file: **' + yearLabel + '** is now the active ledger.'); } catch (e) {}

    return { ok: true, stage: 'ACTIVATED', new_spreadsheet_id: targetSpreadsheetId };
  },

  /**
   * Cancel/discard an in-progress migration.
   */
  cancelMigration: function (actorUserId) {
    var stage = Config.getOptional('MIGRATION_STAGE') || '';
    if (!stage || stage === 'ACTIVATED') return { ok: false, reason: 'No active migration to cancel' };
    ...
    var keysToClear = ['MIGRATION_STAGE', 'MIGRATION_YEAR_LABEL', 'MIGRATION_COMMITTEE_YEAR',
      'MIGRATION_TARGET_SPREADSHEET_ID', 'MIGRATION_TARGET_FOLDER_ID', 'MIGRATION_SELECTIONS'];
    keysToClear.forEach(function(k) { Migration._clearConfig(k); });

    Audit.append(actorUserId, 'Migration', stage, 'CANCELLED', { stage: stage });
   ...
   * @private
   */
  _createTab: function (ss, name, headers) {
...
if (typeof module !== 'undefined') { module.exports = { Migration: Migration }; }
