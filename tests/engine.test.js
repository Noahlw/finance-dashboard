...
global.TABS = { EXPENSE_CLAIMS: 'ExpenseClaims', CLAIM_LINE_ITEMS: 'ClaimLineItems', RECEIPTS: 'Receipts', BUDGET_REQUESTS: 'BudgetRequests', BUDGET_REQUEST_LINES: 'BudgetRequestLines', USERS: 'Users', AUDIT_LOG: 'AuditLog', FINANCE_ACCOUNTS: 'FinanceAccounts', INCOME: 'Income', ACCOUNT_TRANSFERS: 'AccountTransfers', ACCOUNT_ADJUSTMENTS: 'AccountAdjustments', PAYOUTS: 'Payouts' };
global.COLS = {
  ...
  Income: { income_id: 1, date: 2, category_id: 3, amount: 4, received_by: 5, source_ref: 6, event_id: 7, notes: 8, account_id: 9, status: 10, decided_by: 11, decided_at: 12, decision_note: 13, processed_response_id: 14 },
...
describe('Engine._postToAccountBalance', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    realEngine._loadRow = jest.fn(function (entityType, id) {
    ...
    var balance = 0, pending = 0, reserved = 0, activeStatus = 'ACTIVE';
  });
    ...
    realEngine._loadRow = jest.fn(function (entityType, id) {
      if (entityType === 'FinanceAccount' && id === 'AC-001') {
        return {
          ...
          values: ['AC-001', 'Main', 10000, currentBalance, pendingIncome, reservedPayouts, status, '2026-07-01', '']
        };
      }
      return null;
    });
  ...
  it('should increase current_balance for income direction', () => {
    ...
    expect(sheet.getRange).toHaveBeenCalledWith(2, global.COLS.FinanceAccounts.current_balance);
  ...
  it('should decrease reserved_payouts for payout direction', () => {
    ...
    var sheet = realEngine._loadRow.mock.results[0].value.sheet;
    expect(sheet.getRange).toHaveBeenCalledWith(2, global.COLS.FinanceAccounts.reserved_payouts);
  ...
  it('should increase pending_income for pending_income_add', () => {
    ...
    var sheet = realEngine._loadRow.mock.results[0].value.sheet;
    expect(sheet.getRange).toHaveBeenCalledWith(2, global.COLS.FinanceAccounts.pending_income);
...
describe('Engine._computeEffectiveBalance', () => {
  it('should return current_balance + pending_income - reserved_payouts', () => {
    realEngine._loadRow = jest.fn(function (entityType, id) {
      if (entityType === 'FinanceAccount' && id === 'AC-001') {
        return { rowIndex: 2, sheet: {}, values: ['AC-001', 'Main', 10000, 1000, 500, 300, 'ACTIVE', '', ''] };
      }
      return null;
    });
  ...
  it('should handle zero values', () => {
    realEngine._loadRow = jest.fn(function (entityType, id) {
      if (entityType === 'FinanceAccount' && id === 'AC-002') {
        return { rowIndex: 2, sheet: {}, values: ['AC-002', 'Empty', 0, 0, 0, 0, 'ACTIVE', '', ''] };
      }
      return null;
    });
...
describe('Engine.confirmIncome', () => {
  beforeEach(() => {
    ...
    realEngine._loadActor = jest.fn(() => ({ role: 'TREASURER', displayName: 'Treasurer' }));
    realEngine._loadRow = jest.fn(function (entityType, id) {
      if (entityType === 'Income' && id === 'INC-001') {
        return {
          rowIndex: 3,
          sheet: { getRange: jest.fn(() => ({ setValue: jest.fn() })) },
          values: ['INC-001', '2026-07-20', 'CAT-001', 500, 'U-001', 'Tickets', '', '', 'AC-001', 'PENDING', '', '', '', '']
        };
      }
      if (entityType === 'FinanceAccount' && id === 'AC-001') {
        return {
          rowIndex: 2,
          sheet: { getRange: jest.fn(() => ({ setValue: jest.fn() })) },
          values: ['AC-001', 'Main', 10000, 1000, 500, 0, 'ACTIVE', '', '']
        };
      }
      return null;
    });
  ...
  it('should confirm PENDING income and post to balance', () => {
    ...
    expect(result.ok).toBe(true);
    expect(realEngine._postToAccountBalance).toHaveBeenCalledWith('AC-001', 500, 'income');
  });
    ...
    realEngine._loadRow = jest.fn(function (entityType, id) {
      if (entityType === 'Income' && id === 'INC-001') {
        return {
          rowIndex: 3,
          sheet: { getRange: jest.fn(() => ({ setValue: jest.fn() })) },
          values: ['INC-001', '2026-07-20', 'CAT-001', 500, 'U-001', 'Tickets', '', '', 'AC-001', 'CONFIRMED', 'U-002', '2026-07-22', '', '']
        };
      }
      return null;
    });
    ...
    realEngine._loadRow = jest.fn(function (entityType, id) {
      if (entityType === 'Income' && id === 'INC-001') {
        return {
          rowIndex: 3,
          sheet: {},
          values: ['INC-001', '2026-07-20', 'CAT-001', 500, 'U-001', '', '', '', 'AC-001', 'REJECTED', 'U-001', '2026-07-22', '', '']
        };
      }
      return null;
    });
  ...
  it('should require TREASURER role', () => {
    realEngine._loadActor = jest.fn(() => ({ role: 'COMMITTEE', displayName: 'Committee' }));
...
describe('Engine.adjustAccount', () => {
    ...
    realEngine._loadRow = jest.fn(function (entityType, id) {
      if (entityType === 'FinanceAccount' && id === 'AC-001') {
        return {
          rowIndex: 2,
          sheet: { getRange: jest.fn(() => ({ setValue: jest.fn() })) },
          values: ['AC-001', 'Main', 10000, 1000, 0, 0, 'ACTIVE', '', '']
        };
      }
      return null;
    });
  ...
  it('should post credit adjustment and append row', function () {
    global.getSheet_ = jest.fn(() => ({
    ...
    var result = realEngine.adjustAccount('AC-001', 200, 'CREDIT', 'Fix balance', 'U-002');
    expect(result.ok).toBe(true);
    expect(realEngine._postToAccountBalance).toHaveBeenCalledWith('AC-001', 200, 'adjustment_credit');
  });

  it('should post debit adjustment', function () {
    global.getSheet_ = jest.fn(() => ({
      getDataRange: jest.fn(() => ({ getValues: () => [[]] })),
      getLastRow: () => 1,
      getMaxRows: () => 10,
    ...
    expect(realEngine._postToAccountBalance).toHaveBeenCalledWith('AC-001', 100, 'adjustment_debit');
  });

  it('should reject non-treasurer', function () {
    realEngine._loadActor = jest.fn(() => ({ role: 'COMMITTEE', displayName: 'Committee' }));
    var result = realEngine.adjustAccount('AC-001', 100, 'CREDIT', 'Test', 'U-001');
    expect(result.ok).toBe(false);
  });
});
...
describe('Engine.transferBetweenAccounts', () => {
  ...
  it('should debit sender and credit receiver and append row', function () {
    global.getSheet_ = jest.fn(() => ({
      getDataRange: jest.fn(() => ({ getValues: () => [[]] })),
      getLastRow: () => 1,
      getMaxRows: () => 10,
    ...
    var result = realEngine.transferBetweenAccounts('AC-001', 'AC-002', 300, 'Reallocate', 'U-002');
    expect(result.ok).toBe(true);
    expect(realEngine._postToAccountBalance).toHaveBeenCalledWith('AC-001', 300, 'adjustment_debit');
    expect(realEngine._postToAccountBalance).toHaveBeenCalledWith('AC-002', 300, 'adjustment_credit');
  });

  it('should reject non-treasurer', function () {
    realEngine._loadActor = jest.fn(() => ({ role: 'COMMITTEE' }));
    var result = realEngine.transferBetweenAccounts('AC-001', 'AC-002', 100, 'Test', 'U-001');
    expect(result.ok).toBe(false);
  });
});
  ...
  it('should return true when uuid matches', () => {
    var sheet = {
      getDataRange: jest.fn(() => ({
        getValues: () => [
          ['income_id', 'date', 'category_id', 'amount', 'received_by', 'source_ref', 'event_id', 'notes', 'account_id', 'status', 'decided_by', 'decided_at', 'decision_note', 'processed_response_id'],
          ['INC-001', '2026-07-20', 'CAT-001', 500, 'U-001', '', '', '', '', 'PENDING', '', '', '', 'uuid-123']
        ]
      })),
      getLastRow: () => 2
    };
