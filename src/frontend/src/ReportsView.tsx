...
import type { ReportType, ReportFilters, ReportData, SessionRole } from './types';
...
const STATUS_OPTIONS: Record<ReportType, string[]> = {
  claims: ['', 'SUBMITTED', 'NEEDS_INFO', 'VERIFIED', 'REJECTED', 'APPROVED_FOR_PAYOUT', 'PAID', 'LOCKED'],
  budget: ['', 'PENDING', 'NEEDS_INFO', 'APPROVED', 'PARTIALLY_APPROVED', 'REJECTED', 'WITHDRAWN', 'CLOSED'],
...
export default function ReportsView({ role }: { role: SessionRole }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    apiService.getReportsData(activeTab, filters)
  ...
  return (
    <div className="view-container">
      <section className="glass-card">
        ...
        <div className="report-tabs">
          {REPORT_TABS.map(t => (
            <button
              ...
              onClick={() => { setActiveTab(t.type); setShowMigration(false); setFilters({}); }}
            >
              {t.label}
            </button>
          ))}
        ...
        <div className="report-filters">
          ...
          <input
            ...
            placeholder="To"
          />
          <button className="btn btn-secondary" onClick={() => setFilters({})}>Clear</button>
      ...
      {!showMigration && loading && <div className="loader" style={{ margin: '2rem auto' }} />}
        ...
        <section className="glass-card">
          <div className="report-summary">
            ...
            {data.type === 'accounts' && data.total_current_balance != null && (
              <span className="report-balance"> — Total Balance: HKD {data.total_current_balance.toFixed(2)}</span>
            )}
          </div>
  ...
  return (
    <div className="table-scroll">
      <table className="data-table">
        <thead>
          <tr><th>Claim ID</th><th>Claimant</th><th>Status</th><th>Amount</th><th>Submitted</th><th>Event</th><th>Semester</th><th>Method</th></tr>
        </thead>
        <tbody>
          {rows.map((r: any) => (
            <tr key={r.claim_id}>
              ...
              <td className="amount">HKD {Number(r.total_amount || 0).toFixed(2)}</td>
...
function BudgetReportTable({ rows }: { rows: any[] }) {
  ...
  return (
    <div className="table-scroll">
      <table className="data-table">
        <thead>
          <tr><th>Request ID</th><th>Title</th><th>Status</th><th>Requested</th><th>Approved</th><th>Submitted</th><th>Decided</th></tr>
        </thead>
        <tbody>
          {rows.map((r: any) => (
            <tr key={r.request_id}>
              ...
              <td>{r.title}</td>
              <td><span className={`status-chip chip-${(r.status || '').toLowerCase()}`}>{r.status}</span></td>
              <td className="amount">HKD {Number(r.total_requested || 0).toFixed(2)}</td>
              <td className="amount">HKD {Number(r.total_approved || 0).toFixed(2)}</td>
              <td>{r.submitted_at}</td>
              <td>{r.decided_at}</td>
            </tr>
          ))}
...
function IncomeReportTable({ rows }: { rows: any[] }) {
  ...
  return (
    <div className="table-scroll">
      <table className="data-table">
        <thead>
          <tr><th>Income ID</th><th>Date</th><th>Category</th><th>Amount</th><th>Received By</th><th>Source</th><th>Status</th></tr>
        </thead>
        <tbody>
          {rows.map((r: any) => (
            <tr key={r.income_id}>
...
function PayoutsReportTable({ rows }: { rows: any[] }) {
  ...
  return (
    <div className="table-scroll">
      <table className="data-table">
        <thead>
          <tr><th>Payout ID</th><th>Claim ID</th><th>Amount</th><th>Method</th><th>Status</th><th>TXN Ref</th><th>Paid</th></tr>
        </thead>
        <tbody>
          {rows.map((r: any) => (
            <tr key={r.payout_id}>
      ...
      {accounts.length > 0 && (
        <>
          ...
          <div className="table-scroll">
            <table className="data-table">
              <thead>
                <tr><th>Account ID</th><th>Name</th><th>Balance</th><th>Pending Income</th><th>Reserved Payouts</th><th>Status</th></tr>
              </thead>
              <tbody>
                {accounts.map((a: any) => (
                  <tr key={a.account_id}>
                    ...
                    <td>{a.name}</td>
                    <td className="amount">HKD {Number(a.current_balance || 0).toFixed(2)}</td>
                    <td className="amount">HKD {Number(a.pending_income || 0).toFixed(2)}</td>
                    <td className="amount">HKD {Number(a.reserved_payouts || 0).toFixed(2)}</td>
                    <td><span className={`status-chip chip-active`}>{a.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {(data.transfers && data.transfers.length > 0) && (
        <>
          ...
          <div className="table-scroll">
            <table className="data-table">
              <thead>
                <tr><th>Transfer ID</th><th>From</th><th>To</th><th>Amount</th><th>Reason</th><th>Date</th></tr>
              </thead>
              <tbody>
                {data.transfers.map((t: any) => (
                  <tr key={t.transfer_id}>
                    ...
                    <td className="amount">HKD {Number(t.amount || 0).toFixed(2)}</td>
                    <td>{t.reason}</td>
      ...
      {(data.adjustments && data.adjustments.length > 0) && (
        <>
          ...
          <div className="table-scroll">
            <table className="data-table">
              <thead>
                <tr><th>Adjustment ID</th><th>Account</th><th>Amount</th><th>Direction</th><th>Reason</th><th>Date</th></tr>
              </thead>
              <tbody>
                {data.adjustments.map((a: any) => (
                  <tr key={a.adjustment_id}>
                    ...
                    <td className="mono">{a.account_id}</td>
                    <td className="amount">HKD {Number(a.amount || 0).toFixed(2)}</td>
                    <td>{a.direction}</td>
                    <td>{a.reason}</td>
                    <td>{a.adjusted_at}</td>
                  </tr>
